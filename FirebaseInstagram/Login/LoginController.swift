...
//  FirebaseInstagram
//
//  Created by Justin Seymour on 2018/02/15.
...
class LoginController: UIViewController {
    ...
    @objc func handleLogin() {
        ...
        Auth.auth().signIn(withEmail: email, password: password) { (user, err) in
            if let err = err {
                print("Failed to log user in: ", err)
                return
            }
    ...
    fileprivate func setupInputFields() {
        ...
        stackView.anchor(top: logoContainerView.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: 80, paddingLeft: 40, paddingBottom: 0, paddingRight: 40, width: 0, height: 140)
    }
